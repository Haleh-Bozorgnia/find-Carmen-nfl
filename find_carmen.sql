...
AND population = (SELECT MIN(population) FROM country WHERE region = 'Southern Europe');
...
JOIN countrylanguage
ON country.code = countrylanguage.countrycode
WHERE countrylanguage.language ILIKE 'Italian' AND countrylanguage.percentage = 100;
...
WHERE countrycode ILIKE 'smr' AND city.name != 'San Marino';
...
--Find out where she's headed, send us the info, and we'll be sure to meet her at the gates with bells on.
SELECT * FROM city
WHERE population = 91084


-- Carmen is in Santa Monica (California USA)